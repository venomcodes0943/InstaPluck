...
function InstaReelViewer({ requestUrl }) {
  ...
  if (error) {
    return (
      <div
        role="alert"
        className="alert alert-error w-full md:w-96 mx-auto mt-3 px-2 py-1"
      >
        <svg
  ...
  if (!(content === "")) {
    return (
      <video
        className="w-full h-52 md:w-96 mx-auto mt-5 rounded-md shadow-md "
        src={content}
        controls
      />
    );
